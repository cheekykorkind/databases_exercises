...
RSpec.describe Facility do
  describe '#create' do
    context '부모 관계인 ManagementAgency와 PurposeOfCameraInstallation를 미리 만들지 않아서' do
      let(:create_param) {{name: 'facility1', number_of_cameras: 2, management_agency_id: 1, purpose_of_camera_installation_id: 1}}
    ...
    context '부모 관계인 NationalPoliceAgency를 미리 만들면' do
      ...
      let(:management_agency_id) {ManagementAgency.create!({name: "m_agency1", national_police_agency_id:, police_station_id:}).id}
      let(:purpose_of_camera_installation_id) {PurposeOfCameraInstallation.create!({purpose: "purpose1"}).id}
      # Facility
      let(:create_param) {{name: 'facility1', number_of_cameras: 2, management_agency_id:, purpose_of_camera_installation_id:}}
      let(:expected) {1}
      it 'create 가능' do
